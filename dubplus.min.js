...
'use strict';var _waitFor=require('./utils/waitFor.js'),_waitFor2=_interopRequireDefault(_waitFor),_preload=require('./utils/preload.js'),_preload2=_interopRequireDefault(_preload);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}var modal=require('./utils/modal.js'),init=require('./lib/init.js'),css=require('./utils/css.js');function errorModal(a){css.load('/css/dubplus.css'),modal.create({title:'Dub+ Error',content:a})}if(!window.dubplus){(0,_preload2.default)();var checkList=['Dubtrack.session.id','Dubtrack.room.chat','Dubtrack.Events','Dubtrack.room.player','Dubtrack.helpers.cookie','Dubtrack.room.model','Dubtrack.room.users'],_dubplusWaiting=new _waitFor2.default(checkList,{seconds:10});_dubplusWaiting.then(function(){init(),$('.dubplus-waiting').remove()}).fail(function(){Dubtrack.session.id?$('.dubplus-waiting span').text('Something happed, refresh and try again'):errorModal('You\'re not logged in. Please login to use Dub+.')})}else Dubtrack.session.id?errorModal('Dub+ is already loaded'):errorModal('You\'re not logged in. Please login to use Dub+.');
...
"use strict";var GetJSON=require("../utils/getJSON.js"),settings=require("../lib/settings.js");!function(){function a(g,h){return c?void(c.transaction("s").objectStore("s").get(g).onsuccess=function(i){var j=i.target.result&&i.target.result.v||null;h(j)}):void setTimeout(function(){a(g,h)},100)}var b=window.indexedDB||window.mozIndexedDB||window.webkitIndexedDB||window.msIndexedDB;if(!b)return void console.error("indexDB not supported");var c,d={k:"",v:""},f=b.open("d2",1);f.onsuccess=function(){c=this.result},f.onerror=function(g){console.error("indexedDB request error"),console.log(g)},f.onupgradeneeded=function(g){c=null;var h=g.target.result.createObjectStore("s",{keyPath:"k"});h.transaction.oncomplete=function(i){c=i.target.db}},window.ldb={get:a,set:function set(g,h){d.k=g,d.v=h,c.transaction("s","readwrite").objectStore("s").put(d)}}}();var prepEmoji={};prepEmoji.emoji={template:function template(a){return emojify.defaultConfig.img_dir+"/"+encodeURI(a)+".png"}},prepEmoji.twitch={template:function template(a){return"//static-cdn.jtvnw.net/emoticons/v1/"+a+"/3.0"},specialEmotes:[],emotes:{},chatRegex:/:([-_a-z0-9]+):/ig},prepEmoji.bttv={template:function template(a){return"//cdn.betterttv.net/emote/"+a+"/3x"},emotes:{},chatRegex:/:([&!()\-_a-z0-9]+):/ig},prepEmoji.tasty={template:function template(a){return this.emotes[a].url},emotes:{}},prepEmoji.shouldUpdateAPIs=function(a,b){ldb.get(a+"_api",function(d){if(d){var f=JSON.parse(d);"undefined"!=typeof f.error&&b(!0)}var g=Date.now(),h=parseInt(localStorage.getItem(a+"_api_timestamp"));b(isNaN(h)||g-h>5*8.64e7||!d)})},prepEmoji.loadTwitchEmotes=function(){var a;this.shouldUpdateAPIs("twitch",function(b){if(b){console.log("dub+","twitch","loading from api");var c=new GetJSON("https://api.twitch.tv/kraken/chat/emoticon_images","twitch:loaded",{Accept:"application/vnd.twitchtv.v5+json","Client-ID":"z5bpa7x6y717dsw28qnmcooolzm2js"});c.done(function(d){var f=JSON.parse(d),g={};f.emoticons.forEach(function(h){g[h.code]?null===h.emoticon_set&&(g[h.code]=h.id):g[h.code]=h.id}),localStorage.setItem("twitch_api_timestamp",Date.now().toString()),ldb.set("twitch_api",JSON.stringify(g)),prepEmoji.processTwitchEmotes(g)})}else ldb.get("twitch_api",function(d){console.log("dub+","twitch","loading from IndexedDB"),a=JSON.parse(d),prepEmoji.processTwitchEmotes(a),a=null;var f=new Event("twitch:loaded");window.dispatchEvent(f)})})},prepEmoji.loadBTTVEmotes=function(){var a;this.shouldUpdateAPIs("bttv",function(b){if(b){console.log("dub+","bttv","loading from api");var c=new GetJSON("//api.betterttv.net/2/emotes","bttv:loaded");c.done(function(d){var f=JSON.parse(d),g={};f.emotes.forEach(function(h){g[h.code]||(g[h.code]=h.id)}),localStorage.setItem("bttv_api_timestamp",Date.now().toString()),ldb.set("bttv_api",JSON.stringify(g)),prepEmoji.processBTTVEmotes(g)})}else ldb.get("bttv_api",function(d){console.log("dub+","bttv","loading from IndexedDB"),a=JSON.parse(d),prepEmoji.processBTTVEmotes(a),a=null;var f=new Event("bttv:loaded");window.dispatchEvent(f)})})},prepEmoji.loadTastyEmotes=function(){var a=this;console.log("dub+","tasty","loading from api");var b=new GetJSON(settings.srcRoot+"/emotes/tastyemotes.json","tasty:loaded");b.done(function(c){ldb.set("tasty_api",JSON.stringify(c)),a.processTastyEmotes(JSON.parse(c))})},prepEmoji.processTwitchEmotes=function(a){for(var b in a)if(a.hasOwnProperty(b)){var c=b.toLowerCase();if(0<=b.indexOf("\\")){this.twitch.specialEmotes.push([b,a[b]]);continue}if(0<=emojify.emojiNames.indexOf(c))continue;this.twitch.emotes[c]||(this.twitch.emotes[c]=a[b])}this.twitchJSONSLoaded=!0,this.emojiEmotes=emojify.emojiNames.concat(Object.keys(this.twitch.emotes))},prepEmoji.processBTTVEmotes=function(a){for(var b in a)if(a.hasOwnProperty(b)){var c=b.toLowerCase();if(0<=b.indexOf(":"))continue;if(0<=emojify.emojiNames.indexOf(c))continue;0<=b.indexOf("(")&&(c=c.replace(/([()])/g,"")),this.bttv.emotes[c]=a[b]}this.bttvJSONSLoaded=!0,this.emojiEmotes=this.emojiEmotes.concat(Object.keys(this.bttv.emotes))},prepEmoji.processTastyEmotes=function(a){this.tasty.emotes=a.emotes,this.tastyJSONLoaded=!0,this.emojiEmotes=this.emojiEmotes.concat(Object.keys(this.tasty.emotes))},module.exports=prepEmoji;

},{"../lib/settings.js":8,"../utils/getJSON.js":41}],3:[function(require,module,exports){
'use strict';var _createClass=function(){function a(b,c){for(var g,f=0;f<c.length;f++)g=c[f],g.enumerable=g.enumerable||!1,g.configurable=!0,'value'in g&&(g.writable=!0),Object.defineProperty(b,g.key,g)}return function(b,c,f){return c&&a(b.prototype,c),f&&a(b,f),b}}();Object.defineProperty(exports,'__esModule',{value:!0});function _classCallCheck(a,b){if(!(a instanceof b))throw new TypeError('Cannot call a class as a function')}var debugAC=!1;function log(){}var makeList=function(a){function b(l){var m=document.createElement('li');return m.className=l,m}function c(l,m){var n=document.createElement('img');n.src=l,m&&(n.title=m,n.alt=m);var o=document.createElement('div');return o.className='ac-image',o.appendChild(n),o}function f(l){var m=document.createElement('span');return m.textContent=l,m.className='ac-text',m}function g(){var l=document.createElement('span');return l.textContent='press enter to select',l.className='ac-list-press-enter',l}function h(l,m){var p,n=b('preview-item '+l.cn+'-previews'),o=f(l.text);return p=l.alt?c(l.src,l.alt):c(l.src),n.appendChild(p),n.appendChild(o),0===m&&(n.appendChild(g()),n.classList.add('selected')),n.tabIndex=-1,n}var j=document.getElementById('autocomplete-preview');j.innerHTML='';var k=document.createDocumentFragment();a.forEach(function(l,m){k.appendChild(h(l,m))}),j.appendChild(k),j.classList.add('ac-show')};function isElementInView(a,b){var c=a.getBoundingClientRect(),f=b.getBoundingClientRect();return c.top>=f.top&&c.bottom<=f.bottom}var PreviewListManager=function(){function a(b){_classCallCheck(this,a),this._data=b||{start:0,end:0,selected:''}}return _createClass(a,[{key:'repl',value:function repl(b,c,f,g){return b.substring(0,c-1)+g+b.substring(f)}},{key:'updateChatInput',value:function updateChatInput(){log('inUpdate',this._data);var b=$('#chat-txt-message').val(),c=this.repl(b,this._data.start,this._data.end,this._data.selected)+' ';$('#autocomplete-preview').empty().removeClass('ac-show'),$('#chat-txt-message').val(c).focus()}},{key:'doNavigate',value:function doNavigate(b){var c=$('.preview-item.selected').index();c+=b;var f=$('.ac-show li');$('.ac-list-press-enter').remove(),c>=f.length&&(c=0),0>c&&(c=f.length-1);var g='selected';f.removeClass(g).eq(c).addClass(g).append('<span class="ac-list-press-enter">press enter or tab to select</span>');var j=document.querySelector('.preview-item.selected'),k=document.querySelector('#autocomplete-preview'),l=isElementInView(j,k);log('isInView',l);l||j.scrollIntoView(1!==b)}},{key:'updater',value:function updater(b){log(b.target,b),this._data.selected=$(b.target).find('.ac-text').text(),this.updateChatInput()}},{key:'init',value:function init(){var b=this,c=document.createElement('ul');c.id='autocomplete-preview',$('.pusher-chat-widget-input').prepend(c),$(document.body).on('click','.preview-item',function(f){return b.updater(f)})}},{key:'stop',value:function stop(){$('#autocomplete-preview').remove()}},{key:'data',get:function get(){return this._data},set:function set(b){b&&(this._data=b)}},{key:'selected',set:function set(b){b&&(this._data.selected=b)}}]),a}();exports.PreviewListManager=PreviewListManager,exports.makeList=makeList;
...
'use strict';Object.defineProperty(exports,'__esModule',{value:!0});var options=require('../utils/options.js'),dubPlus_modules=require('../modules/index.js'),settings=require('../lib/settings.js'),menu=require('../lib/menu.js'),menuObj={General:'','User Interface':'',Settings:'',Customize:''},loadAllModules=function(){return dubPlus_modules.forEach(function(a){window.dubplus[a.id]=a,window.dubplus[a.id].toggleAndSave=options.toggleAndSave,a.optionState=settings.options[a.id]||!1,'function'==typeof a.init&&a.init.call(a),a.optionState&&'function'==typeof a.turnOn&&a.turnOn.call(a);var b=null;'function'!=typeof a.extra||a.extraIcon||(b='pencil'),menuObj[a.category]+=menu.makeOptionMenu(a.moduleName,{id:a.id,desc:a.description,state:a.optionState,extraIcon:a.extraIcon||b,cssClass:a.menuCssClass||'',altIcon:a.altIcon||null})}),menuObj};exports.default=loadAllModules;

},{"../lib/menu.js":7,"../lib/settings.js":8,"../modules/index.js":29,"../utils/options.js":45}],6:[function(require,module,exports){
"use strict";Object.defineProperty(exports,"__esModule",{value:!0});var options=require("../utils/options.js"),toggleMenuSection=function(a){var b=a.nextElementSibling,c=a.children[0],d=a.textContent.trim().replace(" ","-").toLowerCase(),e="dubplus-menu-section-closed",f=$(b).toggleClass(e).hasClass(e);f?($(c).removeClass("fa-angle-down"),$(c).addClass("fa-angle-right"),options.saveOption("menu",d,"closed")):($(c).removeClass("fa-angle-right"),$(c).addClass("fa-angle-down"),options.saveOption("menu",d,"open"))},traverseMenuDOM=function(a){if(!a||$(a).hasClass("dubplus-menu"))return null;if($(a).hasClass("dubplus-menu-section-header"))return toggleMenuSection(a),null;var b=window.dubplus[a.id];return b?b:traverseMenuDOM(a.parentElement)},menuDelegator=function(a){var b=traverseMenuDOM(a.target);if(b){if($(a.target).hasClass("extra-icon")&&b.extra)return void b.extra.call(b);if(b.turnOn&&b.turnOff){var c;return b.optionState?(c=!1,b.turnOff.call(b)):(c=!0,b.turnOn.call(b)),b.optionState=c,void options.toggleAndSave(b.id,c)}b.go&&b.go.call(b)}};exports.default=function(){var a=document.querySelector(".dubplus-menu");a.addEventListener("click",menuDelegator),document.querySelector(".dubplus-icon").addEventListener("click",function(){$(a).toggleClass("dubplus-menu-open")})};

},{"../utils/options.js":45}],7:[function(require,module,exports){
'use strict';var _menuEvents=require('./menu-events.js'),_menuEvents2=_interopRequireDefault(_menuEvents);function _interopRequireDefault(a){return a&&a.__esModule?a:{default:a}}var settings=require('./settings.js'),css=require('../utils/css.js'),arrow='down',isClosedClass='';'closed'===settings.menu.contact&&(isClosedClass='dubplus-menu-section-closed',arrow='right');var contactSection='\n  <div id="dubplus-contact" class="dubplus-menu-section-header">\n      <span class="fa fa-angle-'+arrow+'"></span>\n      <p>Contact</p>\n    </div>\n    <ul class="dubplus-menu-section '+isClosedClass+'">\n      <li class="dubplus-menu-icon">\n        <span class="fa fa-bug"></span>\n        <a href="https://discord.gg/XUkG3Qy" class="dubplus-menu-label" target="_blank">Report bugs on Discord</a>\n      </li>\n      <li class="dubplus-menu-icon">\n        <span class="fa fa-reddit-alien"></span>\n        <a href="https://www.reddit.com/r/DubPlus/" class="dubplus-menu-label"  target="_blank">Reddit</a>\n      </li>\n      <li class="dubplus-menu-icon">\n        <span class="fa fa-facebook"></span>\n        <a href="https://facebook.com/DubPlusScript" class="dubplus-menu-label"  target="_blank">Facebook</a>\n      </li>\n      <li class="dubplus-menu-icon">\n        <span class="fa fa-twitter"></span>\n        <a href="https://twitter.com/DubPlusScript" class="dubplus-menu-label"  target="_blank">Twitter</a>\n      </li>\n    </ul>';module.exports={beginMenu:function beginMenu(){css.loadExternal('https://maxcdn.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css');var a='<div class="dubplus-icon"><img src="'+settings.srcRoot+'/images/dubplus.svg" alt=""></div>';document.querySelector('.header-right-navigation').insertAdjacentHTML('beforeend',a);return'\n      <section class="dubplus-menu">\n          <p class="dubplus-menu-header">Dub+ Options</p>'},finishMenu:function finishMenu(a,b){for(var c in a){var d=c.replace(' ','-').toLowerCase(),e=settings.menu[d],g='down',h='';'closed'===e&&(h='dubplus-menu-section-closed',g='right'),b+='\n        <div id="'+('dubplus-'+d)+'" class="dubplus-menu-section-header">\n          <span class="fa fa-angle-'+g+'"></span>\n          <p>'+c+'</p>\n        </div>\n        <ul class="dubplus-menu-section '+h+'">',b+=a[c],b+='</ul>'}b+=contactSection,b+='</section>',document.body.insertAdjacentHTML('beforeend',b),(0,_menuEvents2.default)()},makeOptionMenu:function makeOptionMenu(a,b){var d=Object.assign({},{id:'',desc:'',state:!1,extraIcon:null,cssClass:'',altIcon:null},b),e='',f=d.state?'dubplus-switch-on':'';d.extraIcon&&(e='<span class="fa fa-'+d.extraIcon+' extra-icon"></span>');var g='dubplus-switch',h='\n        <div class="dubplus-switch-bg">\n          <div class="dubplus-switcher"></div>\n        </div>';return d.altIcon&&(g='dubplus-menu-icon',h='<span class="fa fa-'+d.altIcon+'"></span>'),'\n      <li id="'+d.id+'" class="'+g+' '+f+' '+d.cssClass+' title="'+d.desc+'">\n        '+e+'\n        '+h+'\n        <span class="dubplus-menu-label">'+a+'</span>\n      </li>'}};
...
'use strict';var modal=require('../utils/modal.js'),options=require('../utils/options.js'),settings=require('../lib/settings.js'),afk_module={};afk_module.id='dubplus-afk',afk_module.moduleName='AFK Auto-respond',afk_module.description='Toggle Away from Keyboard and customize AFK message.',afk_module.category='General',afk_module.canSend=!0;var afk_chat_respond=function(a){if(afk_module.canSend){var b=a.message,c=Dubtrack.session.get('username');-1<b.indexOf('@'+c)&&Dubtrack.session.id!==a.user.userInfo.userid&&(settings.custom.customAfkMessage?$('#chat-txt-message').val('[AFK] '+settings.custom.customAfkMessage):$('#chat-txt-message').val('[AFK] I\'m not here right now.'),Dubtrack.room.chat.sendMessage(),afk_module.canSend=!1,setTimeout(function(){afk_module.canSend=!0},3e4))}};afk_module.turnOn=function(){Dubtrack.Events.bind('realtime:chat-message',afk_chat_respond)},afk_module.turnOff=function(){Dubtrack.Events.unbind('realtime:chat-message',afk_chat_respond)};var saveAFKmessage=function(){var a=$('.dp-modal textarea').val();''!==a&&options.saveOption('custom','customAfkMessage',a)};afk_module.extra=function(){modal.create({title:'Custom AFK Message',content:'Enter a custom Away From Keyboard [AFK] message here',value:settings.custom.customAfkMessage||'',placeholder:'Be right back!',maxlength:'255',confirmCallback:saveAFKmessage})},module.exports=afk_module;

},{"../lib/settings.js":8,"../utils/modal.js":42,"../utils/options.js":45}],10:[function(require,module,exports){
'use strict';var _previewList=require('../emojiUtils/previewList.js'),settings=require('../lib/settings.js'),prepEmjoji=require('../emojiUtils/prepEmoji.js'),debugAC=!1;function log(){}var myModule={id:'dubplus-autocomplete',moduleName:'Autocomplete Emoji',description:'Toggle autocompleting emojis and emotes.  Shows a preview box in the chat',category:'General'},KEYS={up:38,down:40,left:37,right:39,enter:13,esc:27,tab:9,shiftKey:16,backspace:8,del:46,space:32,ctrl:17},keyCharMin=3,inputRegex=/(:)([&!()\+\-_a-z0-9]+)($|\s)/ig,emojiUtils={createPreviewObj:function createPreviewObj(a,b,c){return{src:prepEmjoji[a].template(b),text:':'+c+':',alt:c,cn:a}},addToPreviewList:function addToPreviewList(a){var b=this,c=[],d;a.forEach(function(f){d=f.toLowerCase(),'undefined'!=typeof prepEmjoji.twitch.emotes[d]&&c.push(b.createPreviewObj('twitch',prepEmjoji.twitch.emotes[d],f)),'undefined'!=typeof prepEmjoji.bttv.emotes[d]&&c.push(b.createPreviewObj('bttv',prepEmjoji.bttv.emotes[d],f)),'undefined'!=typeof prepEmjoji.tasty.emotes[d]&&c.push(b.createPreviewObj('tasty',d,f)),0<=emojify.emojiNames.indexOf(d)&&c.push(b.createPreviewObj('emoji',f,f))}),(0,_previewList.makeList)(c)},filterEmoji:function filterEmoji(a){var b=a.replace(/([+()])/,'\\$1'),c=new RegExp('^'+b,'i'),d=emojify.emojiNames;return settings.options['dubplus-emotes']&&(d=prepEmjoji.emojiEmotes),d.filter(function(f){return c.test(f)})}},previewList=new _previewList.PreviewListManager,shouldClearPreview=function(a,b,c,d){var f=c.charAt(c.length-1);return(b.length<d||':'===f||' '===f||''===c)&&(b='',a.innerHTML='',a.className=''),b},handleMatch=function(a,b,c,d){var f=a.length-1,g=a[f].trim(),h=g.charAt(0);g=g.substring(1);var i=b.lastIndexOf(g),j=i+g.length;return log('cursorPos',c),c>=i&&c<=j&&g&&g.length>=d&&':'===h&&emojiUtils.addToPreviewList(emojiUtils.filterEmoji(g)),log('match',a,i,j),{start:i,end:j,currentMatch:g}},chatInputKeyupFunc=function(a){var b=document.querySelector('#autocomplete-preview'),c=0<b.children.length;if(a.keyCode!==KEYS.enter||c){if(a.keyCode===KEYS.up&&c)return a.preventDefault(),void previewList.doNavigate(-1);if(a.keyCode===KEYS.down&&c)return a.preventDefault(),void previewList.doNavigate(1);if((a.keyCode===KEYS.enter||a.keyCode===KEYS.tab)&&c)return a.preventDefault(),previewList.selected=$('.preview-item.selected').find('.ac-text').text(),previewList.updateChatInput(),!1;var d=this.value,f=$(this).get(0).selectionStart,g=d.match(inputRegex),h='';if(g&&0<g.length){var i=handleMatch(g,d,f,keyCharMin);h=i.currentMatch,previewList.data=i}log('inKeyup',previewList.data),shouldClearPreview(b,h,d,keyCharMin)}},chatInputKeydownFunc=function(a){var b=0>=document.querySelector('#autocomplete-preview').children.length,c=_.includes([KEYS.tab,KEYS.enter,KEYS.up,KEYS.down,KEYS.left,KEYS.right],a.keyCode);return c&&b?Dubtrack.room.chat.ncKeyDown({which:a.keyCode}):void(c&&!b&&a.preventDefault())};myModule.turnOn=function(){previewList.init(),Dubtrack.room.chat.delegateEvents(_.omit(Dubtrack.room.chat.events,['keydown #chat-txt-message'])),$(document.body).on('keydown','#chat-txt-message',chatInputKeydownFunc),$(document.body).on('keyup','#chat-txt-message',chatInputKeyupFunc)},myModule.turnOff=function(){previewList.stop(),Dubtrack.room.chat.delegateEvents(Dubtrack.room.chat.events),$(document.body).off('keydown','#chat-txt-message',chatInputKeydownFunc),$(document.body).off('keyup','#chat-txt-message',chatInputKeyupFunc)},module.exports=myModule;
...
'use strict';var settings=require('../lib/settings.js'),modal=require('../utils/modal.js'),options=require('../utils/options.js'),myModule={};myModule.id='chat-cleaner',myModule.moduleName='Chat Cleaner',myModule.description='Automatically only keep a designated chatItems of chat items while clearing older ones, keeping CPU stress down',myModule.category='General',myModule.extraIcon='pencil';var saveAmount=function(){var a=parseInt($('.dp-modal textarea').val());isNaN(a)?options.saveOption('custom','chat_cleaner',500):options.saveOption('custom','chat_cleaner',a)};myModule.chatCleanerCheck=function(){var b=$('ul.chat-main > li').length;isNaN(b)||isNaN(settings.custom.chat_cleaner)||b<settings.custom.chat_cleaner||($('ul.chat-main > li:lt('+($('ul.chat-main > li').length-settings.custom.chat_cleaner)+')').remove(),$('.chat-messages').perfectScrollbar('update'))},myModule.turnOn=function(){Dubtrack.Events.bind('realtime:chat-message',this.chatCleanerCheck)},myModule.extra=function(){modal.create({title:'Chat Cleaner',content:'Please specify the number of most recent chat items that will remain in your chat history',value:settings.custom.chat_cleaner||'',placeholder:'500',maxlength:'5',confirmCallback:saveAmount})},myModule.turnOff=function(){Dubtrack.Events.unbind('realtime:chat-message',this.chatCleanerCheck)},module.exports=myModule;
...
'use strict';var settings=require('../lib/settings.js'),modal=require('../utils/modal.js'),options=require('../utils/options.js'),myModule={};myModule.id='custom_mentions',myModule.moduleName='Custom Mentions',myModule.description='Toggle using custom mentions to trigger sounds in chat',myModule.category='General',myModule.extraIcon='pencil';var saveCustomMentions=function(){var a=$('.dp-modal textarea').val();''!==a&&options.saveOption('custom','custom_mentions',a)};myModule.customMentionCheck=function(a){var b=a.message.toLowerCase();if(settings.custom.custom_mentions){var c=settings.custom.custom_mentions.toLowerCase().split(','),d=c.some(function(f){return 0<=b.indexOf(f.trim(' '))});Dubtrack.session.id!==a.user.userInfo.userid&&d&&Dubtrack.room.chat.mentionChatSound.play()}},myModule.turnOn=function(){Dubtrack.Events.bind('realtime:chat-message',this.customMentionCheck)},myModule.extra=function(){modal.create({title:'Custom Mentions',content:'Add your custom mention triggers here (separate by comma)',value:settings.custom.custom_mentions||'',placeholder:'separate, custom triggers, by, comma, :heart:',maxlength:'255',confirmCallback:saveCustomMentions})},myModule.turnOff=function(){Dubtrack.Events.unbind('realtime:chat-message',this.customMentionCheck)},module.exports=myModule;

},{"../lib/settings.js":8,"../utils/modal.js":42,"../utils/options.js":45}],18:[function(require,module,exports){
'use strict';var settings=require('../lib/settings.js'),modal=require('../utils/modal.js'),options=require('../utils/options.js'),DubtrackDefaultSound='/assets/music/user_ping.mp3',myModule={};myModule.id='dubplus-custom-notification-sound',myModule.moduleName='Custom Notification Sound',myModule.description='Change the notification sound to a custom one.',myModule.category='Customize',myModule.extraIcon='pencil';var saveCustomNotificationSound=function(){var a=$('.dp-modal textarea').val();return''!==a&&a?void(soundManager.canPlayURL(a)?Dubtrack.room.chat.mentionChatSound.url=a:setTimeout(function(){var b=myModule;modal.create({title:'Dub+ Error',content:'You\'ve entered an invalid sound url! Please make sure you are entering the full, direct url to the file. IE: https://example.com/sweet-sound.mp3'}),Dubtrack.room.chat.mentionChatSound.url=DubtrackDefaultSound,b.optionState=!1,b.toggleAndSave(b.id,!1)},100),options.saveOption('custom','notificationSound',a)):(options.saveOption('custom','notificationSound',''),void(Dubtrack.room.chat.mentionChatSound.url=DubtrackDefaultSound))};myModule.extra=function(){modal.create({title:'Custom Notification Sound',content:'Enter the full URL of a sound file. We recommend using an .mp3 file. Leave blank to go back to Dubtrack\'s default sound',value:settings.custom.notificationSound||'',placeholder:'https://example.com/sweet-sound.mp3',maxlength:'500',confirmCallback:saveCustomNotificationSound})},myModule.init=function(){this.optionState&&settings.custom.notificationSound&&this.turnOn()},myModule.turnOn=function(){settings.custom.notificationSound&&''!==settings.custom.notificationSound?Dubtrack.room.chat.mentionChatSound.url=settings.custom.notificationSound:this.extra()},myModule.turnOff=function(){Dubtrack.room.chat.mentionChatSound.url=DubtrackDefaultSound},module.exports=myModule;

},{"../lib/settings.js":8,"../utils/modal.js":42,"../utils/options.js":45}],19:[function(require,module,exports){
'use strict';var _notify=require('../utils/notify.js'),settings=require('../lib/settings.js'),modal=require('../utils/modal.js'),options=require('../utils/options.js'),myModule={};myModule.id='dj-notification',myModule.moduleName='DJ Notification',myModule.description='Notification when you are coming up to be the DJ',myModule.category='General',myModule.extraIcon='pencil';var savePosition=function(){var a=parseInt($('.dp-modal textarea').val());isNaN(a)?options.saveOption('custom','dj_notification',2):options.saveOption('custom','dj_notification',a)};myModule.djNotificationCheck=function(a){if(!(2<a.startTime)){var b=parseInt($('.queue-position').text()),c=0>a.startTime&&!isNaN(b)?b-1:b;isNaN(b)||c!==settings.custom.dj_notification||((0,_notify.showNotification)({title:'DJ Alert!',content:'You will be DJing shortly! Make sure your song is set!',ignoreActiveTab:!0,wait:1e4}),Dubtrack.room.chat.mentionChatSound.play())}},myModule.turnOn=function(){Dubtrack.Events.bind('realtime:room_playlist-update',this.djNotificationCheck)},myModule.extra=function(){modal.create({title:'DJ Notification',content:'Please specify the position in queue you want to be notified at',value:settings.custom.dj_notification||'',placeholder:'2',maxlength:'2',confirmCallback:savePosition})},myModule.turnOff=function(){Dubtrack.Events.unbind('realtime:room_playlist-update',this.djNotificationCheck)},module.exports=myModule;
...
(function (TIME_STAMP){
'use strict';var settings=require('../lib/settings.js'),makeLink=function(a,b){var c=document.createElement('link');return c.rel='stylesheet',c.type='text/css',c.className=a||'',c.href=b,c},load=function(a,b){if(a){var c=makeLink(b,settings.srcRoot+a+'?'+TIME_STAMP);document.head.insertAdjacentElement('beforeend',c)}},loadExternal=function(a,b){if(a){var c=makeLink(b,a);document.head.insertAdjacentElement('beforeend',c)}};module.exports={load:load,loadExternal:loadExternal};

}).call(this,'1511367127232')
...
'use strict';function makeButtons(a){var b='';return a?(b+='<button id="dp-modal-cancel">cancel</button>',b+='<button id="dp-modal-confirm">okay</button>'):b+='<button id="dp-modal-cancel">close</button>',b}var create=function(a){var c=Object.assign({},{title:'Dub+',content:'',value:'',placeholder:null,maxlength:999,confirmCallback:null},a),d='';c.placeholder&&(d='<textarea placeholder="'+c.placeholder+'" maxlength="'+c.maxlength+'">',d+=c.value,d+='</textarea>');var f=['<div class="dp-modal">','<aside class="container">','<div class="title">','<h1>'+c.title+'</h1>','</div>','<div class="content">','<p>'+c.content+'</p>',d,'</div>','<div class="dp-modal-buttons">',makeButtons(c.confirmCallback),'</div>','</aside>','</div>'].join('');document.body.insertAdjacentHTML('beforeend',f),'function'==typeof c.confirmCallback&&$('#dp-modal-confirm').one('click',function(){c.confirmCallback(),$('.dp-modal').remove()}),$('#dp-modal-cancel').one('click',function(){$('.dp-modal').remove()}),$(document).one('keyup',function(g){13===g.keyCode&&'function'==typeof c.confirmCallback&&(c.confirmCallback(),$('.dp-modal').remove()),27===g.keyCode&&$('.dp-modal').remove()})},close=function(){$('.dp-modal').remove()};module.exports={create:create,close:close};
